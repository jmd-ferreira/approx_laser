...
def query_one(filename, output_stream):
    ...
    rules = [
        "poll_inc(MES,SEC) :- time_win(9, 0, 1, @(T, pollution(MES,VAL,SEC)))"
        ...
        "traff_high(MES,SEC) :- time_win(9, 0, 1, diamond(traffic(MES,VAL,SEC)))"
        "and COMP(>=,VAL, 250) and COMP(<=,VAL,300)",

        "poll_low(MES,SEC) :- time_win(9, 0, 1, diamond(pollution(MES,VAL,SEC)))"
        "and COMP(>=,VAL, 0) and COMP(<=,VAL,15)",

        "poll_high(MES,SEC) :- time_win(9, 0, 1, diamond(pollution(MES,VAL,SEC)))"
        "and COMP(>=,VAL, 214) and COMP(<=,VAL,215)",


        "industrial_area(SEC) :- time_win(9, 0, 1, diamond(poll_inc(MES, SEC)))"
        "and time_win(9, 0, 1, diamond(traff_dec(MES10,SEC)))"
        "and time_win(9, 0, 1, diamond(poll_high(MES1,SEC)))"
        "and time_win(9, 0, 1, diamond(traff_low(MES2,SEC)))"
        "and COMP(s!=, MES, MES10)",

        "industrial_box(SEC) :- time_win(3, 0, 1, box(industrial_area(SEC)))",

        "highway_area(SEC) :- time_win(9, 0, 1, diamond(traff_inc(MES, SEC)))"
        "and time_win(9, 0, 1, diamond(poll_inc(MES10,SEC)))"
        "and time_win(9, 0, 1, diamond(poll_high(MES2,SEC)))"
        "and time_win(9, 0, 1, diamond(traff_high(MES4,SEC)))"
        "and COMP(s!=, MES, MES10)",

        "highway_box(SEC) :- time_win(3, 0, 1, box(highway_area(SEC)))",
        ...
        "urban_box(SEC) :- time_win(3, 0, 1, box(urban_area(SEC)))",

        "city(SEC) :- industrial_box(SEC)"
        "and highway_box(SEC)"
        "and urban_box(SEC)",
    ]
...
output_folder = laser_folder + "q1_output/"
