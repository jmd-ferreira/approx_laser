...
class Rule:
    ...
    def doCompSlow(self, l, r, _):
        ...
        if lOprndIsVar:
            ...
            lConstant = float(compNode.atomArgs[0])
        # ----------------------------------------
        rOprndIsVar = compNode.atomArgs[1].isupper()
        if rOprndIsVar:
            rVar = compNode.atomArgs[1]
        else:
            rConstant = float(compNode.atomArgs[1])
        # ----------------------------------------
        lSttVars = lStt.get_column_names()
        retStt = SubstituteTable()
        for v in lSttVars:
...
#    ############################################
    ...
    def holdsAt(self, t, tc, timeline):
        # for rules with empty body return true right away
        if len(self.body) == 0:
    ...
    def rowToAtomString(self, row):
        ...
        if hasVars:
            ...
            for var in headLeafVars:
                # if the variable appears in the mapping, i.e., it is a
                # variable, then use the substitution
                if (var in columnToVarMap):
                    atom += str(row[columnToVarMap[var]])
                # otherwise, use the constant value itself in the result
                else:
                    atom += var
                atom += ","
            atom = atom[:-1]
    ...
    def getCurrentDerivedAtoms(self, now):
        if self.head.nodeType == Node.Atom:
            atoms = {now:set()}
            # for rule facts, just return the element without any substitutions
            if (len(self.body)==0):
                atoms[now].add(self.rowToAtomString(self.head))
            else:
