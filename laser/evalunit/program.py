...
class Program:
    ...
    def deDup(self, strata):
        ...
        cut=0
        while i<(length-cut):
            if tuple(strata[i][1]) not in seen:
                seen.add(tuple(strata[i][1]))
                i += 1
            else:
                strata.pop(i)
                cut += 1
    ...
    def evaluate_strata(self, strata):
        ...
        for predInfo in strata:
            ...
            if isHead:
                rule = predInfo[3]
                # handle the special case of a rule with an empty body - called 
                # with the first parameter 0 for no substitutions
                if (rule.body==[]):
                    for node in predInfo[1]:
                        node.accept(0,now,self.tupleCounter,self.stream.timeLine)
