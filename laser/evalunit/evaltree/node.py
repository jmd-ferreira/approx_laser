...
class Node:
    ...
    #########################################
    def atomAccept(self, inputTuples, now, tupleCounter, __):
        # for rule facts, simply add a tuple without substitutions to the table
        if (inputTuples == 0):
            row = [now,now,tupleCounter, None]
            # then add its arguments
            for arg in self.atomArgs:
                row.append(arg)
